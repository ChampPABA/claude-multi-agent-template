...
- **Large / multi-phase (>~12 nodes, or clearly separable phases):** split into an
  overview page + one detail page per sub-process (see `references/routing.md` §7), and do
...
- **Flow structure (by the book):** every flow begins at exactly one Start terminator and
  ends at one or more End terminators — nothing enters or leaves except through a
  terminator. **Only a decision diamond may fork** (>1 outgoing edge); every other shape
  has exactly one outgoing edge on the happy path. Every connector carries exactly **one
  arrowhead at its target end** — no bidirectional or headless lines.
...
4. Decisions: connect at vertices; happy branch exits toward where flow continues, other
   branches/loops to free vertices (P5).
